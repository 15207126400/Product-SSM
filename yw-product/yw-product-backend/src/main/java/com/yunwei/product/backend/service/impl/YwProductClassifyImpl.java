...
public class YwProductClassifyImpl extends IBaseServiceImpl<YwProductClassify> implements YwProductClassifyService {
	...
	public String uploadClassifyImage(MultipartFile uploadFile,YwProductClassify ywProductClassify) {
		...
		// 上传并保存缩略图
		uploadThumbnail(image_id,uploadFile,ywProductClassify);
		return image_id;
	}
	 ...
	 * @param uploadFile
	 */
	private void uploadThumbnail(final String image_id,final MultipartFile uploadFile,final YwProductClassify ywProductClassify){
		new Thread() {
			...
			public void run() {
				...
				ywImagesBackthumbnail.setThumbnail_name(ywProductClassify.getClassify_name());
				...
				ywImagesXcxthumbnail.setThumbnail_name(ywProductClassify.getClassify_name());
				ywImagesXcxthumbnail.setThumbnail_type("3");
