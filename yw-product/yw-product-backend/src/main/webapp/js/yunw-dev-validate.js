...
function devValidateMobile(sMobile) {
	// 手机号码规则(11位)
	var regMobile = /^(((13[0-9]{1})|(14[0-9]{1})|(15[0-9]{1})|(17[0-9]{1})|(18[0-9]{1})|(19[0-9]{1}))+\d{8})$/;
 ...
 * 小数效验
 */
function devValidatedecimal(s){
	var patrn = /^\d+\.\d+$/;
	var patrn2 = /^0/;
	if(patrn.test(s) && patrn2.test(s)){
		return "success";
	}else{
		return "请输入小数且以0开头,比如0.98";
	}
	return false;
}
 ...
 * @param sMobile
 */
function devValidatefloat(s) {
	var patrn = /^\d*$/;
	var float = /^(-?\d+)(\.\d+)?$/;
	if (patrn.test(s) || float.test(s)) {
		return "success";
	} else {
		return "必须为整数或浮点";
	}
 ...
 * @param sMobile
 */
function devValidateFloatNum(s) {
	var patrn = /^[0-9]+(\.[0-9]{1,2})?$/;
	if (patrn.test(s)) {
		return "success";
	} else {
		return "必须输入合法的数字(包含小数)";
	}
	return false;
}
