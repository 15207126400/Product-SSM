...
<@screen id="ywSmsNotice" title="短信消费记录页面" places=[{"name":"短信消费记录","href":"#"}]>
<@ajaxQueryTable id="ywSmsNotice_list" auto=true isMultiple=true multipleCondition=["sms_id"]
	action="${base}/business/ywSmsNotice/getList.json" 
	paginatorUrl="${base}/business/ywSmsNotice/getPaginator.json"
	...
	<div class="right-btn-box">
	    <#-- <span class="input-group-btn" style="float: left;">
			<a href="${base}/business/ywSmsNotice/edit.htm?op_type=1">
...
<@script>
    function dealStatus(row, head){
        var html = [];
        html.push("<a href='${base}/business/ywSmsNotice/edit.htm?sms_id="+ row["sms_id"] +"&op_type=2' class='tablelink'>查看</a>");
    ...
    // 删除
 	function deleteOne(sms_id,sms_type){
 	    devActAfterConfirmAndClose("您确定要删除吗 ?",function(){
 	       ...
 	       $.ajax({
 		     type:"post",
 		     url:"${base}/business/ywSmsNotice/delete.json",
 		     data:{"sms_id":sms_id},
 		     dataType:"json",
 		     success:function(data){
 		        if(data.error_no == "0"){
 		           devActAfterShowDialog(data.error_info,function(){
	                  location.href = "${base}/business/ywSmsNotice.htm";
	                },"suc");
 		        } else {
 	...
 	function operationCondition(){
	   ...
	   devActAfterConfirmAndClose("您确定要删除吗 ?",function(){
 	   $.ajax({
 		     type:"post",
 		     url:"${base}/business/ywSmsNotice/deleteBatch.json",
