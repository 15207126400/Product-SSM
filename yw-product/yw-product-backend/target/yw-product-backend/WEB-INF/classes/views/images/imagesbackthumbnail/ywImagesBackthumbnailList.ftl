<@screen id="ywImagesBackthumbnail" title="后台缩略图中心中心页面" places=[{"name":"后台缩略图中心中心","href":"#"}]>
<@ajaxQueryTable id="ywImagesBackthumbnail_list" auto=true isMultiple=true multipleCondition=["id"]
	action="${base}/images/ywImagesBackthumbnail/getList.json" 
	paginatorUrl="${base}/images/ywImagesBackthumbnail/getPaginator.json"
	...
	<div class="right-btn-box">
	    <span class="input-group-btn" style="float: left;">
			<a href="${base}/images/ywImagesBackthumbnail/edit.htm?op_type=1">
...
<@script>
    function dealStatus(row, head){
        var html = [];
        html.push("<a href='${base}/images/ywImagesBackthumbnail/edit.htm?id="+ row["id"]+"&op_type=2' class='tablelink'>修改</a>");
    ...
    // 删除
 	function deleteOne(id){
 	    devActAfterConfirmAndClose("您确定要删除吗",function(){
 	       ...
 	       $.ajax({
 		     type:"post",
 		     url:"${base}/images/ywImagesBackthumbnail/delete.json",
 		     data:{"id":id},
 		     dataType:"json",
	...
	function deleteBatch(){
	  var ywImagesBackthumbnailForm = []
	   var id = []
	   ywImagesBackthumbnailForm = getAllCheckboxs().split(",");
	   for(var i=0;i<ywImagesBackthumbnailForm.length;i++){
	   		id.push(ywImagesBackthumbnailForm[i].split("-")[0])
	   }
	  devActAfterConfirmAndClose("您确定要批量删除吗",function(){
		   $.ajax({
				 type:"post",
				 url:"${base}/images/ywImagesBackthumbnail/deleteBatch.json",
				 data:{"id":id.toString()},
				 dataType:"json",
				 success:function(data){
					if(data.error_no == "0"){
