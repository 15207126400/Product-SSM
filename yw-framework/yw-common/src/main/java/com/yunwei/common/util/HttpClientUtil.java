...
/**
 * 功能说明: HttpClient工具类；支持http/https、单向/双向认证；默认执行后自动关闭；<br>
 * 系统版本: v1.0<br>
...
public class HttpClientUtil {
	...
	private boolean isMultipart(Map<String, Object> params) {
		...
		for (Entry<String, Object> entry : params.entrySet()) {
			...
			if (pvalue == null) {
				continue;
			} else if (pvalue instanceof File || pvalue instanceof InputStream) {
				return true;
	 ...
	 */
	public byte[] httpPost(String url, Map<String, Object> params, List<Header> headers) throws ClientProtocolException, IOException {
	...
	/**
	 * POST方式请求，支持http/https，支持提交其他格式(json,xml)
	 * @param url
	 * @param xml/json
	 * @param headers
	 * @return
	 ...
	 * @throws ClientProtocolException
	 */
	private byte[] execute(HttpRequestBase request, List<Header> headers) throws ClientProtocolException, IOException {
		...
		try {
			...
			if (response.getStatusLine().getStatusCode() == HttpServletResponse.SC_OK) {
				...
				HttpEntity entity = response.getEntity();
				byte[] data = null;
				try { data = EntityUtils.toByteArray(entity); } catch (Exception ignore) {}
				data = data == null ? ("网络请求发生异常:" + response.getStatusLine() + " URI=" + request.getURI()).getBytes() : data;
				logger.warn("网络请求发生异常:" + FastJsonUtil.toJSONString(new String(data)));
				throw new BizException(String.valueOf(response.getStatusLine().getStatusCode()), "网络请求发生异常:" + response.getStatusLine() + " URI=" + request.getURI());
			}
		} finally {
			if (request != null) {
				try {
