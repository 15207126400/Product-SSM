...
public class ImageCodeServiceImpl implements IImageCodeService {
	...
	public String getRandString() {
		...
		String validateCodeType = "number";
	 ...
	 */
	@Override
	public void saveValidateCode(String verifyCode,HttpSession session) {
...
//		}
		...
		logger.info("sessionid新：{}",session.getId());
...
//		}
		cachedCode = (String) session.getAttribute(session.getId());
		logger.info("sessionid旧：{}",session.getId());
		logger.info("原有验证码：{}",verifyCode);
		logger.info("session中的验证码：{}",cachedCode);
